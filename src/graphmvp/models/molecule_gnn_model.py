...
class GNN_graphpred(nn.Module):
    def __init__(self, args, num_tasks, molecule_model=None):
        ...
        for t in range(self.num_tasks):
            if self.JK == "concat":
                self.pred_layers.append(nn.Linear(self.mult * (self.num_layer + 1) * self.emb_dim, 1))
            else:
                self.pred_layers.append(nn.Linear(self.mult * self.emb_dim, 1))
        return
    ...
    def get_graph_representation(self, *argv):
        ...
        preds = []
        for t in range(self.num_tasks):
            pred = self.pred_layers[t](graph_representation)
            preds.append(pred)
    ...
    def encode(self, loader, device):
        self.eval()
        encodes = []
        with torch.no_grad():
            for batch in loader:
                batch = batch.to(device)
                encode, _ = self.get_graph_representation(batch)
